...
export const unarchiveNote = async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const noteId = parseInt(req.params.id as string, 10);
  ...
  const result = await pool.query(
    'UPDATE notes SET archived = FALSE, updated_at = NOW() WHERE id = $1 AND user_id = $2 RETURNING *',
    [noteId, userId]
  );
...
export const pinNote = async (req: AuthRequest, res: Response) => {
  ...
  const result = await pool.query(
    'UPDATE notes SET pinned = TRUE, updated_at = NOW() WHERE id = $1 AND user_id = $2 RETURNING *',
    [noteId, userId]
  );
...
export const unpinNote = async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const noteId = parseInt(req.params.id as string, 10);
  ...
  const result = await pool.query(
    'UPDATE notes SET pinned = FALSE, updated_at = NOW() WHERE id = $1 AND user_id = $2 RETURNING *',
    [noteId, userId]
  );
